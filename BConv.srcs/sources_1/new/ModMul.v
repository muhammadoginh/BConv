...
module ModMul #(
    ...
    delay #(.BW(BW), .N(6)) delay_q (.clk(clk), .in(q), .out(q_d));
    assign {q2, q1, q0} = q_d;
...
//    pipeline #(2*m+3) pipe_w5_sub (clk, rstn, w5_sub, w5_sub_pipe);
    ...
    delay #(.BW(BW), .N(3)) delay_q_final (.clk(clk), .in(q_d), .out(q_d2)); // might be problem
    ...
    always @(posedge clk) begin
        if (!rstn) begin
            ...
            if (t >= {q_d2,1'b0}) begin
                M_reg <= t - {q_d2,1'b0};
            end else if (t >= q_d2) begin
                M_reg <= t - q_d2;
            end else begin
                M_reg <= t;
            end
